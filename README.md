...
- **글머리 기호로 시작하는 줄에서 낱말 하나가 조각날 수 있습니다.** 번역 엔진이 문단을 나눌 때 낱말 안쪽에서 끊는 일이 있습니다. 앞 조각은 되붙여 번역하지만, 뒤에 남은 한두 글자(`l` 같은)는 번역 엔진이 우리에게 넘기지 않아 그대로 찍힙니다. 원본 슬라이드에는 없던 글자입니다.
...
- **바닥글의 쪽 번호가 날짜 쪽으로 끌려옵니다.** 같은 높이에 떨어져 있던 `2023-03-15` 와 `9` 를 번역 엔진이 한 덩어리로 묶어 다시 조판하기 때문입니다. 쪽 번호가 구분선 위에 겹쳐 보일 수 있습니다. 실측으로 23쪽 중 20쪽. 엔진 쪽 동작이라 우리가 넘기는 설정으로는 바뀌지 않습니다 (`--no-merge-alternating-line-numbers` 를 켜 봐도 같았습니다).
