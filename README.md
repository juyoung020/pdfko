...
- **낱말이 한가운데서 갈라져 보일 수 있습니다.** 번역 엔진이 `Agent` 를 `Age` + `nt` 로 끊어 따로 배치하면 `Age  nt` 처럼 사이가 벌어집니다. 잘린 조각은 번역하지 않고 그대로 두므로(`Age` 를 `연령` 으로 옮기는 일은 없습니다) 읽을 수는 있지만, 두 조각을 다시 붙일 방법은 없습니다 — 엔진이 각 조각의 자리를 직접 정합니다. 실측으로 23쪽에 2건.
- **동그라미 숫자(①②③)가 보통 숫자로 바뀝니다.** 번역 엔진이 글자를 다시 그릴 때 그 글꼴에 없는 문자를 대체합니다. 번역을 끄고 조판만 하면 그대로 남는 것으로 확인했고, `--primary-font-family` 를 바꿔도 같았습니다.
