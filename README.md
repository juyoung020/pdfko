...
분야를 가리지 않습니다. **도구에 용어 목록이 들어 있지 않습니다.** 문서에서 자주 나오는 말을 추려 모델에게 "이 중 무엇이 이 책의 전문 용어냐"고 묻고, 모델이 고른 것만 씁니다. 강화학습 교재든 생물학 교재든 같은 방식으로 동작합니다.

같은 8쪽을 세 가지로 번역해 **표기가 갈린 용어 수**를 셌습니다.

| | 표기가 갈린 용어 |
|---|---|
| 통일 안 함 | 1개 (가치 함수 / 값 함수) |
| 손으로 만든 용어집 108개 | 2개 (업데이트 / 갱신, 과제 / 작업) |
| **자동 통일 (기본값)** | **0개** |

손으로 만든 용어집이 오히려 나빴습니다. 지정한 역어가 모델이 자연스럽게 쓰는 말과 달라서 둘이 섞였습니다. 자동 통일은 모델이 이미 쓰는 말을 고정하므로 그런 충돌이 없습니다.
